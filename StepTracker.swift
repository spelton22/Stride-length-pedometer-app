...
class StepTracker: ObservableObject {
    ...
    @Published var totalDistance: Double = 0.0 // in feet
    ...
    @Published var speedOutlierThreshold: Int = 3
    //@Published var speedTolerance: Float = 0.2
    //@Published var strideLengthTolerance: Float = 0.15 // in meters
    //@Published var strideLengthRaw_print: Float = 0.0
    //@Published var strideLengthEmpirical_print: Float = 0.0
    ...
    var empiricalK: Double = 0.37 // Tunable constant for empirical model
    ...
    @Published var gyro_x: Double = 0.0
    @Published var gyro_y: Double = 0.0
    @Published var gyro_z: Double = 0.0
    
    //for graph
    @Published var stepTimestamps: [TimeInterval] = []
    @Published var stepZAccelerations: [Float] = []
    @Published var stepIntervals: [Double] = []
    
    struct StepChartDataPoint: Identifiable {
        let id = UUID()
        let stepIndex: Int
        let zAcceleration: Float
        let timeInterval: Double
    }
    
    var chartData: [StepChartDataPoint] {
        let count = min(stepZAccelerations.count, stepIntervals.count)
        return (0..<count).map { i in
            StepChartDataPoint(stepIndex: i + 1,
                               zAcceleration: stepZAccelerations[i],
                               timeInterval: stepIntervals[i])
        }
    }
        ...
        if accelerationHistory.count >= 3 {
            ...
            if b > a && b > c && b > 0.9 { // Peak threshold
                ...
                if timeSinceLastStep > 0.3 { // Debounce: ~max 3 steps/sec
                    ...
                    stepZAccelerations.append(b)
                    
                    // Save time interval
                    if let last = stepTimestamps.last {
                        stepIntervals.append(currentTime - last)
                    }
    ...
    private func addStepData(stepLength: Float, speed: Float) {
        ...
        if last10StepOutliers >= stepOutlierThreshold || last10SpeedOutliers >= speedOutlierThreshold {
...
//        if difference > strideLengthTolerance {
//            stepManager.triggerWarning()
//        }
...
//        strideLengthEmpirical_print = Float(strideLengthEmpirical)
    ...
    private func calculateAverages() {
        ...
        let suffix = strideLengthRawValues.suffix(10)
        averageStrideLengthRaw = suffix.isEmpty ? 0 : suffix.reduce(0, +) / Double(suffix.count)
        let empiricalStrideLengthAvg = strideLengthEmpiricalValues.suffix(10).reduce(0, +) / 10.0
        
        //averageStrideLengthRaw = rawStrideLengthAvg
        averageStrideLengthEmpirical = empiricalStrideLengthAvg
